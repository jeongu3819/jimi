...
export function HUD({ state, onPause }: Props) {
  ...
  const isDark = state.lightLevel < 0.3;
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.center}>
        {state.players.map(p => (
          <div key={p.id} style={{ ...styles.ps, opacity: p.alive ? 1 : 0.35, borderColor: p.color }}>
            ...
            {p.alive && <span style={{ color: '#aaa', fontSize: 9 }}>E:{p.elevation}</span>}
            {p.alive && p.isClimbing && <span style={{ color: '#ffaa44', fontSize: 9 }}>CLIMB</span>}
      ...
      <div style={styles.right}>
        ...
        {isDark && <span style={{ color: '#555', fontSize: 10 }}>{'\u{1F319}'}</span>}
...
const styles: Record<string, React.CSSProperties> = {
  container: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 10px', background: 'rgba(0,0,0,0.8)', borderBottom: '1px solid rgba(255,50,50,0.1)', minHeight: 36, gap: 6, flexWrap: 'wrap' },
  ...
  wave: { color: '#ff6644', fontSize: 12, fontFamily: 'monospace', fontWeight: 'bold' },
